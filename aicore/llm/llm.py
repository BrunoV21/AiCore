from pydantic import BaseModel, RootModel, model_validator, computed_field, Field
...
class Providers(Enum):
    """Enum mapping provider names to their implementation classes."""
    ...
    def get_instance(self, config: LlmConfig) -> LlmBaseProvider:
        ...
        Example:
            ...
            >>> provider = Providers.OPENAI.get_instance(config)
        """
...
class Llm(BaseModel):
    ...
    @staticmethod
    def _include_reasoning_as_prefix(prefix_prompt: Union[str, List[str], reasoning: str) -> List[str]:
    ...
    def _reason(self, 
            ...
            action_id: Optional[str] = None) -> List[str]:
        ...
        if self.reasoner:
            ...
            reasoning = self.reasoner.provider.complete(
                prompt, system_prompt, prefix_prompt, img_path, False, stream, agent_id, action_id
            )
            reasoning_msg = REASONING_INJECTION_TEMPLATE.format(
                reasoning=reasoning, 
                reasoning_stop_token=REASONING_STOP_TOKEN
            )
    ...
    async def _areason(self, 
        ...
        if self.reasoner:
            ...
            reasoning = await self.reasoner.provider.acomplete(
                prompt, sys_prompt, prefix_prompt, img_path, False, stream, self.logger_fn, agent_id, action_id
            )
            reasoning_msg = REASONING_INJECTION_TEMPLATE.format(
                reasoning=reasoning, 
                reasoning_stop_token=REASONING_STOP_TOKEN
    ...
    def complete(
        ...
        return self.provider.complete(
            prompt, sys_prompt, prefix_prompt, img_path, json_output, stream, agent_id, action_id
        )
    ...
    async def acomplete(
        ...
        return await self.provider.acomplete(
            prompt, sys_prompt, prefix_prompt, img_path, json_output, stream, self.logger_fn, agent_id, action_id
        )
