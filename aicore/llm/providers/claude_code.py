"""
Claude Code provider for AiCore.

Uses the `claude-agent-sdk` `query()` function — stateless, fresh session per
call, internal tool chaining via `max_turns`.  Matches AiCore's stateless design.

Auth is handled entirely by the Claude Code CLI; no API key is required.
temperature and max_tokens are ignored for this provider.
"""
...
class ClaudeCodeLlm(LlmBaseProvider):
    ...
    # ------------------------------------------------------------------
    # Config validation is a no-op — auth is handled by the CLI
    # ------------------------------------------------------------------
